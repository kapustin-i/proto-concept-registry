...
**Specification, v0.2 — working draft**
...
The father, the family, the sea — let us leave them on the beach. I turn to myself, or rather to the question the reader already has: what has any of this to do with him? Who is writing, and why does it concern him?

I am not an engineer. Doctor, barber, police officer, driver — it does not matter. What matters is this: it is dripping on my head and into my plate.

I do not know how language models are built inside, and I do not presume to advise on how to build them. There is not one place in this document where I know better than a specialist. I am not a roofer: I know nothing of the joints, the materials, or how a leak is patched. But it leaks on me, and so I look up to see where from. The roofer knows the roof. I know where it lands. These are different kinds of knowledge, and the second is not part of his trade.

Quite possibly the roofer is sitting on the next stool — and the same hole is dripping on him, except that at this moment he is looking at the rafters.

Nobody here lied. The machine does not know that it does not know. The person does not know that it does not know. The engineer saw water on the floor, put a bucket under it, and was right to: no puddle, nobody slipped, the task is closed. The defect is not in the solution but in its boundary. There is no more water on the floor — and the hole is still there, and there is no longer any reason to ask about it. An answer that removes the symptom removes the question with it.

No culprit is needed for this. It is enough that it drips.

What is new here, for me, is neither the roof nor the machine. What is new is that I am told "there is nothing" in the same voice that tells me "here is a fact," and I have no way to tell the two apart. That is what this document is about. The observation is mine. The solutions are yours.
...
### 1.1 The remainder and the re-labeling failure mode

Everything a system matches against the known goes into the answer. Everything unmatched is the remainder. The remainder holds three different things: **noise** (a sensor twitched, interference, chance — there is nothing to reduce it to); **known but unrecognized** (the word exists in the world and among people, but not in this system); and **the nameless** (no one has a word, the phenomenon has not yet been isolated as a phenomenon). From inside the system these three are indistinguishable: all look identical — matched nothing. They can be separated only from outside, and by different means: noise falls away under repetition, the second by consulting catalogues and people, the third remains when there is no one left to consult. The first two stages were solved before this document and are surveyed in §2. What follows concerns only the third kind of remainder.
...
Scale changes not only the fraction here but the nature of the damage, and it does so in both directions.

**Uniformity as a multiplier.** For a human being, an interpretation error costs one case: one watcher, one beach, one second. For a system placed across an entire stream, the same defect executes uniformly and simultaneously at every point of deployment. The issue is not that the model errs more often than a person — it is that it errs *identically*, and the cost of the error is multiplied by reach.

**That same reach is the only available foothold.** A single glint is indistinguishable from a trick of the light: separating them requires understanding what one is looking at. A thousand glints in one square are separable arithmetically — by recurrence, by agreement among independent channels, by stable binding to place and time — and no understanding is required at all. A mass occurrence of the nameless is therefore detectable *without* being recognized, which moves the task from the philosophical register into the engineering one: counting frequency is what such systems already do. This is exactly what the registry is for (§3). It does not supply an organ for "I have no name for this"; it accumulates single, inconsequential signals until they reach the density at which they become visible as a frequency.

**And the honest limit of this.** The statistical route catches only what recurs. A singular event — a glint that happened once and never came again — it will never catch, by construction. The watcher in the prologue remains uncovered: there was one event there. This document promises the detectability of the mass nameless, not of every instance of it.
...
There is a second reason this failure passes unnoticed, and it is a human one. People have grown used to model errors: a user knows a citation may be invented and corrects for it — checks what is doubtful, double-checks what matters. The habit formed quickly and works well. But it works in one direction only: what can be checked is what has been produced — open the link, recompute the figure, find the quotation. The correction is bound to an artifact. A reported absence has no artifact: "nothing found" does not look like an error, it looks like a result, and an unremarkable one at that. The habit of doubt, trained on the invented, does not fire here at all — it has nothing to catch on. The closure is therefore double: no trace on the machine's side, no occasion on the human's. This is why negative confabulation has no statistics — not because it is rare, but because each instance ends in nothing and is recorded by no one.
...
People do grow used to the leak, and this is not faint-heartedness but sound economy: living under a bucket is cheaper than climbing onto the roof. Exhortation therefore does not work here — the remainder of this document is not about vigilance but about the form of the report.

But the hole has a sequel, and it is no longer a conjecture. Every instance in which the nameless was called the known leaves behind not emptiness but text — fluent, coherent, plausible. Such text enters the common corpus and becomes material on which the next generation of systems is trained. It is known that training on one's own output degrades the distribution: the rare is washed out, answers converge to the average (Shumailov et al. 2024). The effect at issue here is a different one: the nameless, once recorded as the known, enters the corpus already under a borrowed name. What degrades is not answer quality but the very possibility of telling a finding apart from the background.

And what drowns is not "information." What drowns is a person — the one whose case does not resemble the recorded ones, whose trouble has not been described, whose remedy is in no handbook. Everything that arrives in the world without a name arrives as a weak signal. The father on the beach could not tell the flash of glasses from the ripples — and that remains this document's subject: what drowns in the ripples is not a signal, it is a person.
...
**A precedent from another trade: near-miss reporting.** The problem of making a system report something that ended in nothing and has no name has already been solved — in aviation, and after it in medicine, nuclear power, and industrial safety. The solution was not to cultivate vigilance but to engineer the report: a near-miss report is cheap to file, non-punitive, and mandatory; the vast majority of filings lead nowhere, and that is the norm rather than a failure; value appears in the statistics, when the same nameless oddity arrives from several crews. Such forms carry a field for "event type: unclassified" — precisely what is missing from a language model's output. What transfers here: cheap registration, non-punishment, obligation, provenance, value in aggregate rather than per item. What does not transfer, and is flagged in §5: in aviation the reporter is a human being who felt a jolt, whereas here the reporter is a machine that feels none; and the whole construction rests on obligation backed by a regulator, whereas here there is no one to oblige.

**A precedent of conduct: the plate that was not thrown out.** The institutional precedents above answer the question of how to arrange a process. There is an earlier one, answering what an observer should do in the second of the encounter itself. Mold suppressing bacterial growth had been seen on laboratory plates long before 1928; the plates were discarded, and the notebook entry was not "I observe something unexplained" but *contaminated* — a report of an absence. Fleming did exactly three things, none of which required understanding the mechanism: he did not take the free ready-made word; he recorded the observation with provenance — conditions, plate, effect (Fleming 1929); and he attached no name he could not justify. The referent was supplied by others and later: Florey and Chain retrieved the paper a decade on, isolated the substance, and reproduced the effect independently. In the terms of §3.2 this is the full route `registered → candidate → adopted`, executed by history, with two independent parties at the final step and a ten-year gap between registration and legalization — a gap treated here as the normal speed, not a failure.

It does not follow that the procedure existed then. Fleming acted from character, not protocol, which is precisely why the case is singular rather than typical. What follows is this, and only this. If a human being did it once, the requirement is executable in principle. Fleming needed judgment for it — but only because he had a single plate. A system standing across a stream needs no judgment: what a lone observer settled by instinct, a multiplicity settles by counting (§1.1). And once the requirement reduces to *do not re-label, and record*, it is programmable. This document asks a system for exactly that, and for nothing beyond it.
...
A status track describes states but not movement between them, and in v0.1 this left the quarantine a **shelf**: an entry sat in `registered` waiting for someone to bring a referent test. The wait could be indefinite, and for most entries it would be. The quarantine is better built as a **cycle**: the machine hands out not its own puzzlement but a passport for the finding — matched nothing; recurred this many times; corroborated by this many independent channels; bound to this place and time; no addressee in the catalogues; here is the fragment. Looking at the passport, a person says where to look harder. The system goes and looks exactly there, and the passport is updated. The cycle has three honest exits: **matched** (it was known — the entry closes), **held** (the feature reproduced — a candidate for a name), **dissolved** (under targeted checking the signal did not recur — it was noise). The referent test is therefore not an entry ticket to quarantine but its **product**: it is forged across several iterations rather than required up front.

The status formulation follows from this. "No match found" is not a property of the phenomenon, nor a feature by which it is recognized, but a protocol result: as of this date, across this coverage of catalogues, no correspondence was found. That makes the status falsifiable — it is lifted by anyone who produces a match. An entry therefore asserts nothing about the world; it presents, and it is constructed to be as cheap as possible to close. An entry's weight grows by exactly the number of attempts to close it that have failed.
...
The cost of registration: on constrained hardware an entry is not free — memory, energy, writes — so a threshold appears, and **the rule deciding what never even enters quarantine becomes the most load-bearing part of the design**; that is where "just a trick of the light" quietly takes up residence. The obligation to report: the aviation precedent (§2) rests on filing being unavoidable and unpunished, backed by a regulator — here there is no one to oblige, and voluntary registration loses to a fluent answer on price. The absence of the author's own measurements: none of this document's claims is supported by an experiment actually run, including the proposed paraphrase-divergence test. The side of the failure: the prologue describes a failure in the **watcher**, not in the victim — the technical consequence is unworked, and may lie not in the model but in a recorder of weak ambiguous signals held by whoever is looking. Routing and prioritization of whispers to name-givers; incentive design for filing referent tests (who pays for ground contact); anchor portability across architectures (an embedding is model-local — what survives transposition?); adversarial registration (flooding the registry as an attack); and the honest hard case — distinctions real in a model's activation geometry with no worldly referent at all, which the quarantine correctly never promotes but which may still matter for interpretability.

## 6. Status of this document

v0.2, working draft; request for comment. Added relative to v0.1: the author's position; the remainder split into three kinds (§1.1); the human-side reason the failure goes unnoticed (§1.2); the corpus-contamination effect (§1.3); the near-miss reporting precedent (§2); quarantine as a cycle and "no match found" as a protocol of exhaustion (§3.2); the name-giver as spotter (§3.3); four open problems (§5). The two filtering stages of the remainder — noise models and catalogue search — are explicitly acknowledged as solved before the author; this document's claim begins after them. The author's prior specification in this genre: *Offline Emergency Dispatch Protocol* (github.com/kapustin-i/offline-dispatch-protocol). Same intent here: not a product, a floor for one — published so it cannot be quietly unpublished.
...
- Fleming, A. *On the antibacterial action of cultures of a penicillium.* Br J Exp Pathol 10 (1929).
